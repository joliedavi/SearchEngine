...
// that come in useful throughout.
...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  if (realpath(test_file.c_str(), file_path_buf) == NULL) {
    return false;
  }

  // Get full rootdir path
  char rootdir_path_buf[PATH_MAX];
  if (realpath(root_dir.c_str(), rootdir_path_buf) == NULL) {
    return false;
  }

  // See if rootdir is within full file path
  if (string(file_path_buf).find((string(rootdir_path_buf) + "/")) == 0) {
    return true;
  }
  return false;  // you may want to change this return value
}
